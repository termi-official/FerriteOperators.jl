...
mappings and both per-quadrature-point `storage` elections.

The element math is the pointwise map ``W_q = w_q \det(J_q) J_q^{-1} D
J_q^{-T}``; everything around it is
[`AbstractTensorProductElementCache`](@ref)'s, whose scope (tensor-product
Lagrange on `RefQuadrilateral`/`RefHexahedron`, distorted cells included) is
this integrator's.
...
# the scratch is the one per-worker field.
...
# the cell's coordinates; a stored level reads a factor addressed by the cell id
# alone. The store's presence is a type parameter, so the coordinate staging
# folds away with it.
...
# which carries no compile-time symmetry guarantee. Checking the VALUE once, at
# `ElementAssemblyCache` construction, is the conservative form of the election
# ([`element_matrix_symmetry`](@ref)): true for every `D` this cache can be
# built from, and automatically false should a non-symmetric diffusion tensor
# be threaded through.
#
...
#
# Packing always wins bytes/cell and wins ACTION time only at p = 1; the docs'
# "Assembly levels" section carries the measured trade-off.
...
# by the lane that produced it, so it needs neither storage nor a barrier.
# `Stored()`: the same expression, evaluated once by the fill sweep below.
